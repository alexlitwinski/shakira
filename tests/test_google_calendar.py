...
def test_store_save_link(monkeypatch, tmp_path):
    monkeypatch.setenv("SHAKIRA_USER_DATA_ROOT", str(tmp_path))
    phone = "5511999999999"
    email = "user@gmail.com"
    cid = base64.b64encode(email.encode()).decode().rstrip("=")
    url = f"https://calendar.google.com/calendar/u/0?cid={cid}"
    from app.google_calendar_actions import handle_google_calendar_save_link

    reply = handle_google_calendar_save_link({"calendar_public_url": url}, phone)
    assert "guardado" in reply.lower() or "Link" in reply
